...
pub async fn retrieve_file(hash: &str, conn: DbConn, width: Option<usize>, height: Option<usize>) -> Result<CachedImage, Flash<Redirect>> {
        ...
        Some(f_schema) => {
            if width.is_some() && height.is_some() {
                ...
                if image_width as i32 > f_schema.width || image_height as i32 > f_schema.height {
                    ...
                    let rimg = img.unwrap().resize(width.unwrap() as u32, height.unwrap() as u32, image::imageops::FilterType::Lanczos3);
...
pub async fn get_thumbnail_folder(conn: DbConn, folder: &str, width: Option<usize>, height: Option<usize>, cache: &State<ImageCache>) -> Result<CachedImage, Flash<Redirect>> {
    ...
    let file_schema_vec = FileSchema::random(&conn, folder.to_string(), 1).await;
    let file_schemas = file_schema_vec.unwrap();

    match file_schemas.first() {
        None => Err(Flash::error(Redirect::to("/"), "file not found.")),
        Some(f_schema) => {
            let image_width = width.unwrap_or(100);
            let image_height = height.unwrap_or(100);
                ...
                true => {
                    match image::open(&f_schema.path) {
                        Ok(thumb_dynamic_image) => {
                            let t_dynamic_image = thumb_dynamic_image.resize(width.unwrap() as u32, height.unwrap() as u32, image::imageops::FilterType::Lanczos3);
