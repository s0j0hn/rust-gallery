...
pub async fn random_json(
    ...
    match FileSchema::random(&conn, folder.unwrap_or("*").to_string(), random_size as i64).await {
        Ok(files) => {
...
pub async fn get_all_json(
    ...
    match FileSchema::all_paged(&conn, pper_page as i64, offset as i64, folder.unwrap_or("*").to_string()).await {
        Ok(files) => {
            let pimages = JsonFileResponse {
                items: files.clone(),
                page: ppage,
