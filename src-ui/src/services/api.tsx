...
import {
  ...
  JsonResponseIndex,
  JsonRootResponse,
  JsonRootResponseSchema,
  validateApiResponse,
} from '../schemas/gallery.schema'
import { z } from 'zod'
import { JsonResponse } from '@/types/gallery.tsx'
...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError) => {
      ...
      switch (error.response.status) {
        case 400:
          return Promise.reject(
            new Error('Invalid request. Please check your input and try again.')
          )
        case 401:
          return Promise.reject(new Error('You need to log in to perform this action.'))
        case 403:
    ...
    return Promise.reject(
      new Error('An error occurred while connecting to the server. Please try again later.')
    )
  }
)
...
export const api = {
  config: {
    getConfig: async (): Promise<JsonConfig> => {
      ...
      return validateApiResponse(JsonConfigSchema, response.data)
    },
    updateConfig: async (params: JsonConfig): Promise<void> => {
      // Validate input before sending
      const validatedParams = validateApiResponse(JsonConfigSchema, params)
      const response = await apiClient.post('/config', validatedParams)
      if (response.data.status === 'error') {
        throw new Error('Error while updating the config: ' + response.data.error)
      }
    },
  },
  ...
  folders: {
    ...
    getRoots: async (): Promise<JsonRootResponse[]> => {
      ...
      return validateApiResponse(z.array(JsonRootResponseSchema), response.data)
    },
    ...
    getAll: async (
      ...
      const sanitizedRoot = root ? sanitize.folderName(root) : 'files'
      const sanitizedSearchBy = searchBy ? sanitize.string(searchBy) : undefined
    ...
    getByName: async (folderName: string): Promise<Folder | null> => {
      ...
      const response = await apiClient.get(`/folders/json/name/${sanitizedName}`)
    ...
    // Update album tags
    updateTags: async (folderName: string, tags: string[]): Promise<string[]> => {
      const sanitizedName = sanitize.folderName(folderName)
  ...
  photos: {
    ...
    getRandomByTag: async (tag: string, size: number): Promise<JsonResponse<JsonFilePhoto[]>> => {
      let params = {}
      if (tag) {
