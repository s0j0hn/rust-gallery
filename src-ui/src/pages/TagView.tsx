  ...
  useEffect(() => {
    ...
    const fetchPhotos = async () => {
      ...
      try {
        ...
        const data = await api.photos.getRandomByTag(tagName, config.photo_per_random)
        // Photos received
    ...
    return (
      <div className="min-h-screen bg-gray-100">
        <div className="container mx-auto p-4">
          <MenuSection onTagClick={handleTagClick} onApiDocsClick={handleApiDocsClick} />
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="container mx-auto p-4">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-3xl font-bold">{`Photos tagged with "${tagName}"`}</h1>
          </div>

          {photos.length > 0 ? (
            <PhotoSwipeGallery images={photos} />
          ) : (
            <div className="text-center py-8">No photos found with this tag.</div>
          )}

          {/* Show total count if available */}
          {photos.length > 0 && (
            <div className="text-center text-gray-600 mt-4">Showing {photos.length} photos</div>
          )}
        </div>
      </div>
    </div>
