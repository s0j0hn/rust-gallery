    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-lg">
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <p>
                <strong>Estimated Size:</strong> {formatCacheSize(stats.size)}
              </p>
            </div>
            <div>
              <p>
                <strong>Oldest Cached:</strong> {formatDate(stats.oldestEntry)}
              </p>
              <p>
                <strong>Newest Cached:</strong> {formatDate(stats.newestEntry)}
              </p>
            </div>
          </div>
        </div>
        ...
        {cachedItems.length > 0 && (
          <div className="mb-6">
            <h3 className="font-semibold mb-2">Recent Cached Thumbnails</h3>
            <div className="grid grid-cols-3 gap-2">
              {cachedItems.map((item) => (
                <div key={item.key} className="relative overflow-hidden rounded h-20 bg-gray-200">
                  <img
                    src={item.url}
