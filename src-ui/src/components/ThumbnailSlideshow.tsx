...
const ThumbnailSlideshow: FC<ThumbnailSlideshowProps> = ({ thumbnails, folderName }) => {
      ...
      try {
        ...
        for (let i = 1; i <= 2; i++) {
          ...
          if (!controller.signal.aborted) {
            preloadPromises.push(preloadImage(thumbnails[nextIndex], folderName))
          }
        }
        ...
        if (!controller.signal.aborted) {
          preloadPromises.push(preloadImage(thumbnails[prevIndex], folderName))
        }
