...
export const GalleryContent: FC<{
  ...
  useEffect(() => {
    // Only try to open if we have images and need to open immediately
    if (openNow && images.length > 0) {
      // Delay the opening to ensure refs are properly set up
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1">
      {images.map((image) => (
        <Item
          ...
          width={image.width}
          alt={`${image.filename}.${image.extension}`}
          height={image.height}
              ...
              <div className="relative pb-full">
                <img
                  ...
                  className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${
                    isImageLoaded(image.hash) ? 'opacity-100' : 'opacity-0'
                  }`}
