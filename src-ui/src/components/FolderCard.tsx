...
const FolderCard: FC<FolderCardProps> = memo(
  ({ folder, onView, onTagClick, onTagManage, onRandomPhoto, onDelete }) => {
    ...
    // Fetch tags for this folder using the existing useTagsQuery hook
    const { data: tags = [], isLoading: tagsLoading } = useTagsQuery(folder.title)

    // Use fetched tags or fallback to folder.tags
    const displayTags = tags.length > 0 ? tags : folder.tags
        ...
        <div className="p-4">
          <div className="flex justify-between items-start">
            <h2 className="text-xl font-semibold mb-2">
              {searchQuery ? highlightText(folder.title, searchQuery) : folder.title}
            </h2>
            <button
  ...
  (prevProps, nextProps) => {
    ...
    // Since we're fetching tags dynamically, we only compare the core folder properties
    // Tags will be handled by React Query's caching and re-renders
    return (
